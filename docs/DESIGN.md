...
- `text` would declare `['sidecar']` and `supportsDirectives: false`, and `mdx` all three strategies.
...
| File renderer | Genuinely extensible today via `plugins.renderers` (chapter 6): a renderer is a `parse`/`stringify` pair over the same mdast tree every pipeline stage uses. Only `markdown` is built in so far. |
...
- **`text`/`mdx` renderers**: v1 only implements the `markdown` renderer; `.txt`/`.mdx` support is design-stage only.
