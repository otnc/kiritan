...
- **`@kiritan/free-translate`**([`packages/free-translate`](../packages/free-translate)): 対応済み。APIキー不要のミドルウェア集で、すべて `@kiritan/middleware` の上に作られている: `myMemory()`(公開されている無料API。1リクエスト500バイトのため、この層の `measure` オプションでバイト単位に分割する)、`googleFree()`(非公式の `clients5.google.com` の `dict-chrome-ex` エンドポイント。まとめ送信に対応し、未知の言語コードには原文がそのまま返るため、コードを事前に検証する)、`apertium()`(公開のAPyサーバー。ヨーロッパ系の言語のみで日本語は無い)、`libreTranslate({ baseUrl })`(自前ホスト)。どのプロバイダーを作るかは実際に試して決めた: MyMemory、Apertium、Chrome拡張機能のエンドポイントは実機から使えた一方、素の `client=gtx` エンドポイントはCAPTCHAに当たり、LingvaはCloudflareの向こう側にあり、Microsoft Edgeの翻訳認証は何も返さず、libretranslate.comは今やキーが必須だった。応答する3つは、実際のMarkdownのサンプル(コードフェンス、インラインコード、リンク、`%{name}`、見出し、リスト)で最初から最後まで実行しており、その実行で見出しの `# ` やリストの記号が空白を失うことが分かったため、この層は行頭の記号も保護している。`libreTranslate` はサーバーが無かったため、公式のAPI仕様に対するユニットテストのみ。`googleFree` のエンドポイントは非公式でブロックされたり変わったりする可能性があり、READMEにもそう明記している。
