...
:::kiritan{locale=en}
The design intent for `mdx`, once it's implemented: a Markdown extension. JSX parts would never be translated, `:::kiritan{...}` could be used the same way as in markdown, and it would declare all three strategies.
:::
:::kiritan{locale=ja}
...
| File renderer | Genuinely extensible today via `plugins.renderers` (chapter 6): a renderer is a `parse`/`stringify` pair over the same mdast tree every pipeline stage uses. `markdown` and `text` are built in so far. |
...
- **`mdx` renderer**: v1 has no `mdx` support yet, design-stage only.
:::
:::kiritan{locale=ja}
