...
- `text`, `mdx`, `plugins.renderers` for custom renderers, and the strategy/renderer compatibility validation described below are all still design-stage, not implemented in v1 (tracked in chapter 13).
:::
:::kiritan{locale=ja}
...
- `text`・`mdx`・カスタムレンダラー用の `plugins.renderers`、および後述のstrategy/レンダラー組み合わせ検証は、いずれもまだ設計段階でv1では未実装(13章で追跡)。
:::
