...
interface KiritanConfig {
  ...
  markdown?: { alertBlankLineSpaces?: number }; // Trailing spaces on the blank lines inside GitHub alerts. Default: 0 (a bare `>`)
  plugins?: {
...
interface KiritanConfig {
  ...
  markdown?: { alertBlankLineSpaces?: number }; // GitHubアラート内の空行に付ける末尾スペースの数。既定: 0(素の `>`)
  plugins?: {
    stores?: Record<string, TranslationStore>;
    renderers?: Record<string, Renderer>;
  };
...
- **`machine` markers and stale detection for `sidecar` and `inline`**: done. Chapter 7 always said auto-translated content is marked `machine` ("a field for `catalog`, a comment marker for `sidecar`/`inline`"), but only `catalog` was. Now `kiritan translate` writes a `kiritan:machine` comment line under a `sidecar` translation's hash marker, and `kiritan check` reports `machine` until a reviewer deletes it (a renderer with no comment syntax, i.e. `text`, gets no marker and so no review flag, as it never had a hash). `inline` gained the rest: `kiritan translate` used to throw for it, since a translation has no file of its own. It now writes into the base file, by source offset so nothing else is reformatted: locale blocks written next to each other (nothing else between them) form a group, a locale missing from a group gets a block after it, and a block that carries a `hash` is rewritten when the default-locale block it came from has changed. A written block is `:::kiritan{locale=ja machine hash=<hash of the source block>}` (the same number of colons as its source, so nested directives still work): `machine` is the review flag, and `check` reports a `hash` mismatch as `stale`. A block with no `hash` was written by hand and is never touched or flagged. `build` ignores both attributes.
- **`markdown.alertBlankLineSpaces`**: done. The serializer writes the blank line inside a GitHub alert as a bare `>`, which can't satisfy a house style that wants `>` plus trailing spaces (this repo's own is exactly three). The option pads those lines after the fact, only inside alert blocks and never inside a fenced code block within one. It defaults to 0 (a bare `>`); this repo sets it to 3, so its generated READMEs now match the style of their sources.
- **Dependency policy (ESM/CJS)**: decided. Anything a consumer loads through a package's API must be a library whose latest release is ESM/CJS-dual (see CONTRIBUTING); the CLI may use ESM-only ones. Audited across the repo: `@kiritan/middleware`, the provider packages and `@kiritan/runtime` comply (`@lezer/markdown`, `es-toolkit`, `limiter`, `html-entities`, `ofetch`; `jiti` and `tinyglobby` in `kiritan`). The exception is `kiritan`'s `unified`/`remark-*` stack, which is ESM-only and whose stringifier has no dual-format replacement; it stays, bundled into `dist/`, so CJS consumers are unaffected. Replacing it would mean rewriting the directive/interpolation/switcher pipeline as source-range edits on `@lezer/markdown`'s positions and finding a different way to support `.mdx` (`remark-mdx` has no substitute), and would change how generated documents are formatted; that is left for when it is worth doing. The rest of the code was checked for hand-written logic a library could replace: `deepMerge` (config layering) was left alone, since `defu`'s array and null semantics differ from the `mergeArray` contract and the saving is a few lines.
...
- **`@kiritan/middleware`** ([`packages/middleware`](../packages/middleware)): done. `createTranslator({ name, translate | translateBatch, ... })` turns a bare "translate this text" function into a complete batch-form middleware, so a provider (or a user's own code) only writes the call itself. It exists because a middleware receives raw Markdown — a whole file for `sidecar`, one directive body for `catalog` — and a bare provider call mishandles that: it translates code, URLs, front matter and `:::kiritan` lines, and a long README exceeds the provider's length limit. The layer (1) swaps protected spans for `[[N]]` tokens and refuses a result that lost one (the token shape was picked by testing against Google's and MyMemory's real endpoints, which rewrote `<span translate="no">` and split `XPH0X`-style words); (2) splits a text over `maxChars` at paragraph boundaries and rejoins it with the original spacing, and packs short ones into `translateBatch` calls under `maxBatchSize`/`maxBatchChars`; (3) applies concurrency, request spacing and retries (network errors and 408/409/425/429/5xx only); (4) caches finished translations, in memory or in a JSON file; (5) with `onError: "skip"` leaves only failing texts to the next middleware. The Markdown is parsed with `@lezer/markdown` (GFM included) instead of matched with patterns: every non-prose range the parser reports is shielded, so structure such as tables, nested fences, setext headings, reference links and HTML blocks is handled by the parser rather than by a regex list that would have to track Markdown. Retries and concurrency use `es-toolkit`, request spacing `limiter`, sentence/word/grapheme cuts `Intl.Segmenter`, and locale parsing `Intl.Locale`. It has no dependency on `kiritan` (structural types). `@kiritan/deepl` and `@kiritan/google-translate` are built on it too: the layer's `wire` option lets each wrap the `[[N]]` tokens in its provider's own "leave this alone" markup (DeepL's ignored XML tag, Google's `translate="no"` span) while the layer does the protecting, splitting and retrying. Their size limits are measured on the text as actually sent (escaped, wrapped and, for DeepL, JSON-encoded), so a whole README sent for a `sidecar` translation is split rather than rejected.
- **`@kiritan/free-translate`** ([`packages/free-translate`](../packages/free-translate)): done. Middlewares that need no API key, all built on `@kiritan/middleware`. Hosted: `myMemory()` (documented free API, 500 bytes per request, so it uses the layer's `measure` option to split by bytes) and `googleFree()` (the unofficial `clients5.google.com` `dict-chrome-ex` endpoint; batched; validates language codes because the endpoint returns the source text unchanged for an unknown one). Self-hosted: `libreTranslate({ baseUrl })` (the open-source engine on a server you run; array `q`, so batched) and `appsScript({ url, secret })` (a small Google Apps Script web app you deploy from your own account, using the built-in `LanguageApp.translate`; the README carries the script to paste in, with the deployment steps and the two ways it usually goes wrong: not deployed for "Anyone", or edited without a new version). Apps Script answers a POST with a 302 to a one-off result URL, so the provider is tested against a real local HTTP server that answers that way, not only a fake `fetch`. Which hosted providers to build was decided by trying them from a real machine: MyMemory and the Chrome-extension endpoint worked, while the plain `client=gtx` endpoint hit a CAPTCHA, Lingva sat behind Cloudflare, Microsoft Edge's translate auth returned nothing, and libretranslate.com requires a key. An Apertium provider (public server, European languages only, no Japanese) was built and then removed as useless for the languages most Kiritan projects need. `googleFree`, `myMemory` and the layer were run end to end against the live services on a Markdown sample (front matter, heading, bold, list, quote, table, code fence, an entity, `%{name}`). `libreTranslate` and `appsScript` are verified against their documented request/response shapes only: no LibreTranslate server or Apps Script deployment was available to run them against. `googleFree` is unofficial and may be throttled, blocked or change; the README says so.
...
- **`sidecar` と `inline` の `machine` マーカーとstale検知**: 対応済み。7章は一貫して、自動翻訳の内容は `machine` としてマークされる(「`catalog` はフィールド、`sidecar`/`inline` はコメントマーカー」)と述べていたが、実際にそうなっていたのは `catalog` だけだった。`kiritan translate` は `sidecar` の翻訳のhashマーカーの下に `kiritan:machine` のコメント行を書くようになり、レビュー担当者がそれを削除するまで `kiritan check` が `machine` を報告する(コメント構文を持たないレンダラー、つまり `text` は、もともとhashも持たないので、マーカーも付かずレビューフラグも立たない)。`inline` は残りが揃った: これまで `kiritan translate` は、翻訳を置く専用のファイルが無いため例外を投げていた。今はbaseファイル自身に、ソースのオフセットで書き込むので、それ以外の部分は整形し直されない。隣り合って書かれた(間に他の内容が無い)ロケールブロックが1つのグループになり、グループに無いロケールにはその後ろにブロックが追加され、`hash` を持つブロックは、翻訳元の既定ロケールのブロックが変わっていれば書き直される。書き込まれるブロックは `:::kiritan{locale=ja machine hash=<元のブロックのhash>}`(元と同じ数のコロンなので、入れ子のディレクティブも問題ない)で、`machine` がレビューフラグ、`hash` の不一致は `check` が `stale` と報告する。`hash` の無いブロックは手書きなので、決して触られず、フラグも立たない。`build` はどちらの属性も無視する。
- **`markdown.alertBlankLineSpaces`**: 対応済み。シリアライザーはGitHubのアラート内の空行を素の `>` で書くため、`>` に末尾スペースを付ける作法(このリポジトリでは正確に3つ)を満たせなかった。このオプションは、そうした行を後から埋める。対象はアラートのブロック内だけで、その中のフェンスコードの内側には触れない。既定は0(素の `>`)で、このリポジトリは3にしているため、生成されるREADMEもソースの作法に揃う。
- **依存のポリシー(ESM/CJS)**: 決定済み。パッケージのAPIを通じて利用者が読み込むものは、最新版がESM/CJS両対応のライブラリにする(CONTRIBUTINGを参照)。CLIはESM専用のものを使ってよい。リポジトリ全体を調査した結果: `@kiritan/middleware`・各プロバイダーのパッケージ・`@kiritan/runtime` は適合している(`@lezer/markdown`、`es-toolkit`、`limiter`、`html-entities`、`ofetch`。`kiritan` では `jiti` と `tinyglobby`)。例外は `kiritan` の `unified`/`remark-*` 群で、ESM専用であり、そのstringifierに両対応の代替が無い。これは残し、`dist/` にbundleするため、CJSの利用者には影響しない。置き換えるには、ディレクティブ・補間・switcherのパイプラインを `@lezer/markdown` の位置情報に基づくソース範囲の編集として書き直し、`.mdx` を別の方法で支える必要があり(`remark-mdx` に代替は無い)、生成ドキュメントの整形も変わるため、やる価値が出てきた時に回す。それ以外のコードも、ライブラリで置き換えられる手書きのロジックが無いか確認した: `deepMerge`(設定のレイヤー合成)は、`defu` の配列・nullの扱いが `mergeArray` の契約と異なり、削減できるのも数行なので、そのままにした。
