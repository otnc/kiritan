...
:::kiritan{locale=en}
`kiritan init` leaves every file it would write alone if it already exists (`--force` overwrites) — safe to run again in a project that already has some of the three set up. A per-command `--locale` flag (restricting a run to one locale) is still design-stage, not implemented in v1 (tracked in chapter 13).
:::
:::kiritan{locale=ja}
`kiritan init` は書き込み先のファイルが既に存在する場合はそのまま残す(`--force` で上書き) — 3つのうち一部だけ既に用意されているプロジェクトでも再実行して安全。各コマンド共通の `--locale` フラグ(実行対象を1ロケールに絞る)は、まだ設計段階でv1では未実装(13章で追跡)。
:::
